...
public:
  ...
  void dexp(mp_exp_t k_div, mp_exp_t k_mod){
	...
	const int m = 32 - nlz(r_d[size - 1] >> (32 - n));
	mp_limb_t last = r_d[size - 1] >> (32 - n);
    for(int i = size - 1; i > 0; --i){
      r_d[i] = (r_d[i] << n) | (r_d[i - 1] >> (32 - n));
    }
	r_d[0] <<= n;
	if(last > 0){
		++r_d;
  ...
  mp_exp_t r2_exp() const{
	...
	const int a_size = a.mp->_mp_size < 0 ? -a.mp->_mp_size : a.mp->_mp_size;
    return (a_e - 1) * 32 + 32 - nlz(a_d[a_size - 1]);
  }
...
private:
  static int nlz(mp_limb_t x){
    if(x == 0){ return 32; }
	int n = 0;
    if(x <= 0x0000FFFF){ n = n + 16; x <<= 16; }
    if(x <= 0x00FFFFFF){ n = n + 8; x <<= 8; }
    if(x <= 0x0FFFFFFF){ n = n + 4; x <<= 4; }
    if(x <= 0x3FFFFFFF){ n = n + 2; x <<= 2; }
